  ...
  public static function create_cron() {
    if ( WDSeo()->is_active() ) {
      if ( defined( 'TENWEB_INCLUDES_DIR' ) ) {
        ...
        if ( true === TenwebServices::manager_ready() ) {
          $crawl = new WD_SEO_CRAWL;
          $crawl_errors = $crawl->get_crawl_errors(0);
          $google_authorized = isset($crawl_errors['error']) && $crawl_errors['error'] === TRUE && !isset($crawl_errors['message']) ? FALSE : TRUE;

          $moz = new WD_SEO_MOZ;
          $moz_url_metrics = $moz->get_url_metrics();
          $moz_authorized = $moz_url_metrics === FALSE ? FALSE : TRUE;

          $data[ "body" ] = array(
            'url' => WD_SEO_REST_API_CRAWL,
            'domain_id' => TenwebServices::get_domain_id(),
            'moz_authorized' => $moz_authorized,
  ...
  public static function custom_redirects_init( $args = array() ) {
    ...
    if ( !empty($redirects) ) {
      foreach ( $redirects as $redirect ) {
        ...
        $should_redirect = ($current_url === trailingslashit(trailingslashit($site_url) . $url)) || ($current_url === trailingslashit($url));
